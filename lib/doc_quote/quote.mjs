...
class DocQuote extends HTMLElement {
	...
	#createInfoElm() {
		...
		// イベントを処理して要素の表示トグルを OFF にします。
		function toggleOff(event) {
			if (event.target !== window && result.contains(event.target)) {
				return;
			}
	...
	#jumpTo(elm) {
		Doc.scrollTo(elm);
		Doc.reserveOnetimeAnimation(animationElm(elm), "var(--jumpAnim)");
