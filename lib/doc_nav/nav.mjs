...
class DocNav extends HTMLElement {
	...
	#importSectionHeaders() {
		...
		function freshSections() {
			...
			const nextIdx = last ? Util.lastIndexOf(sections, last) + 1 : 0;
			return Array.prototype.slice.call(sections, nextIdx);
		}
		...
		function reserveUlElm(stackDepth) {
			...
			for (let i = 0; i < stackDepth; i++) {
				...
				if (!lastUl) {
					return lastLi.appendChild(document.createElement("ul"));
				} else {
					target = lastUl;
				}
			}
...
class NavUtil {
	...
	static jumpTo(elm) {
		...
		elm.scrollIntoView();
		animation("none");
		window.requestAnimationFrame(animation.bind(null, "var(--jumpAnim)"));
...
class Util {
	...
	static lastIndexOf(arr, target) {
		return Array.prototype.lastIndexOf.call(arr, target);
	}
	...
	static observeElmLoading(elm, callback) {
		...
		document.addEventListener("DOMContentLoaded", exit, true);
		...
		function exit() {
			callback(_ => {});
			document.removeEventListener("DOMContentLoaded", exit, true);
			observer.disconnect();
		}
	}
