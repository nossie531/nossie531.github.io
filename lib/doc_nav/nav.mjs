...
class DocNavSub extends HTMLElement {
	...
	#importSectionHeaders() {
		const self = this;
		const areaSection = NavUtil.getOwnerSection(this);
		for (const section of freshSections()) {
			if (NavUtil.getOwnerSection(section.parentElement) !== areaSection) {
				continue;
			}
		...
		function freshSections() {
			...
			const sections = areaSection.getElementsByTagName("section");
			const freshCnt = last ? Util.lastIndexOf(sections, last) : sections.length;
			return Array.prototype.slice.call(sections, -freshCnt);
		}
	...
	#setupElmLoadingObserve() {
		const areaSection = NavUtil.getOwnerSection(this);
		Util.observeElmLoading(areaSection, _ => this.#importSectionHeaders());
	}
}
...
class NavUtil {
	...
	// 要素が紐づくセクション要素を取得。
	static getOwnerSection(elm) {
		return Util.findAncestor(elm, x => !!NavUtil.getSectionHeader(x));
	}
