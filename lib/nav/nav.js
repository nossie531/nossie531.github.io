...

{
	...
	class NavManager {
		...
		#setupRelatedLinks() {
			...
			const promise = Util.fetchXhtml(indexUrl);
			...
			function getAnchorUrl(a) {
				if (!a) {
					return null;
				} else {
					return new URL(a.getAttribute("href"), indexUrl).toString();
				}
			}
			...
			function findOffsetUrl(indexDoc, href, offset) {
				...
				const currIndex = anchors.findIndex(x => getAnchorUrl(x) === document.URL);
				const targetIndex = currIndex >= 0 ? currIndex + offset : -1;
				return getAnchorUrl(anchors[targetIndex]);
			}
