...

{
	...
	class Nav {
		...
		static exec() {
			const nav = new Nav;
			Util.callbackDefer(nav.#setup.bind(nav));
		}
		...
		#setupHeaderIds(element, contextId) {
			let sectionId = 0;
			for (const section of Nav.#getNavSections(element)) {
				const secHeader = Nav.#getSectionHeader(section);
		...
		// 目次要素を生成。
		#createIndex(element) {
			const sections = Nav.#getNavSections(element);
		...
		static #getNavSections(element) {
			...
			for (let elm = tw.nextNode(); elm;) {
				...
				const h1Elm = Nav.#getSectionHeader(scElm);
